<template>
  ...
  <div v-if="loading" class="text-center py-20">
    <p class="text-lg text-gray-500">Chargement du profil...</p>
  </div>

  <!-- Profile content -->
  <div v-else class="min-h-screen bg-gradient-to-br from-white via-gray-50 to-blue-50 py-8">
    <div class="max-w-4xl mx-auto px-6">
        ...
        <div class="flex flex-col md:flex-row items-center md:items-start gap-8">
          ...
          <div class="relative">
            <div class="w-32 h-32 rounded-full bg-gradient-to-r from-blue-600 to-blue-500 flex items-center justify-center text-4xl text-white font-bold shadow-lg">
              <span v-if="!profile.avatar && profile.username">
          ...
          <div class="flex-1 text-center md:text-left">
            ...
            <p class="text-instagram-text-light mb-4 text-lg">
        ...
        <div v-if="editing" class="mt-8 pt-8 border-t border-instagram-border animate-fade-in">
          ...
          <form @submit.prevent="saveProfile" class="space-y-6">
            ...
            <div class="flex gap-3">
              <button type="submit" class="btn-instagram">Enregistrer</button>
              <button type="button" @click="editing = false" class="btn-instagram-outline">Annuler</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div class="animate-slide-up">
        ...
        <div v-if="userPosts.length === 0" class="text-center py-16">
          ...
          <p class="text-instagram-text-light">
            {{ isMe ? 'Créez votre première publication !' : 'Cet utilisateur n\'a pas encore publié.' }}
          </p>
        </div>
...
const profile = computed(() =>
  userStore.getUserByUsername(username) || { username, bio: '', avatar: '', following: [] }
)
...
const userPosts = computed(() =>
  postStore.posts.filter(p => p.author === username)
)
